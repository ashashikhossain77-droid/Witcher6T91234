/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { LineEntry } from '../types';
import { buildDebonairLineEntry, DebonairRowInput } from './debonairDataHelper';

export const DEBONAIR_SEPTEMBER_19_DATE = '2026-09-19';

const RAW_ROWS_SEP_19: DebonairRowInput[] = [
  // PADMA FLOOR (Lines 1 - 6)
  {
    lineNo: '1',
    chief: 'BAYZED',
    buyer: 'BEST SELLER',
    style: 'OSJ BOWIE SOFTSHELL BOMBER',
    smv: 39.49,
    daysRun: 1,
    wh: 9.48,
    mp: 58,
    dayTarget: 251,
    dayProduction: 99,
    planEff: 30,
    achvEff: 12,
    effPrf: 40,
    availableMin: 33000,
    produceMin: 3969,
    remarks: 'HERBERT LIGHT PADDED HOOD 22 pcs'
  },
  {
    lineNo: '2',
    chief: 'A.RAUF',
    buyer: 'OTCF',
    style: 'JAF 1156',
    smv: 45.20,
    daysRun: 10,
    wh: 10.00,
    mp: 57,
    dayTarget: 454,
    dayProduction: 240,
    planEff: 60,
    achvEff: 32,
    effPrf: 53,
    availableMin: 34200,
    produceMin: 10848
  },
  {
    lineNo: '3',
    chief: 'TUHIN',
    buyer: 'H&M',
    style: 'QUINN LIGHT BOMBER JKT',
    smv: 28.49,
    daysRun: 10,
    wh: 10.25,
    mp: 60,
    dayTarget: 777,
    dayProduction: 600,
    planEff: 60,
    achvEff: 46,
    effPrf: 77,
    availableMin: 36900,
    produceMin: 17094
  },
  {
    lineNo: '4',
    chief: 'AKTAR',
    buyer: 'H&M',
    style: 'QUINN LIGHT BOMBER JKT',
    smv: 28.49,
    daysRun: 24,
    wh: 10.00,
    mp: 59,
    dayTarget: 746,
    dayProduction: 10,
    planEff: 60,
    achvEff: 1,
    effPrf: 1,
    availableMin: 35400,
    produceMin: 285
  },
  {
    lineNo: '5',
    chief: 'MOHASIN',
    buyer: 'H&M',
    style: 'DANNY MID PADDED JKT',
    smv: 46.76,
    daysRun: 116,
    wh: 10.00,
    mp: 62,
    dayTarget: 557,
    dayProduction: 555,
    planEff: 70,
    achvEff: 70,
    effPrf: 100,
    availableMin: 37200,
    produceMin: 25952
  },
  {
    lineNo: '6',
    chief: 'HAMID',
    buyer: 'H&M',
    style: 'DANNY MID PADDED JKT',
    smv: 46.76,
    daysRun: 58,
    wh: 10.00,
    mp: 62,
    dayTarget: 557,
    dayProduction: 541,
    planEff: 70,
    achvEff: 68,
    effPrf: 97,
    availableMin: 37200,
    produceMin: 25297
  },

  // MEGHNA FLOOR (Lines 7 - 12)
  {
    lineNo: '7',
    chief: 'Anisur',
    buyer: 'OTCF',
    style: 'JAF 434',
    smv: 41.02,
    daysRun: 58,
    wh: 10.00,
    mp: 61,
    dayTarget: 535,
    dayProduction: 330,
    planEff: 60,
    achvEff: 37,
    effPrf: 62,
    availableMin: 36600,
    produceMin: 13537
  },
  {
    lineNo: '8',
    chief: 'SAGOR',
    buyer: 'OTCF',
    style: 'JAM 1144',
    smv: 43.15,
    daysRun: 12,
    wh: 10.00,
    mp: 60,
    dayTarget: 501,
    dayProduction: 415,
    planEff: 60,
    achvEff: 50,
    effPrf: 83,
    availableMin: 36000,
    produceMin: 17907
  },
  {
    lineNo: '9',
    chief: 'RUBEL',
    buyer: 'BEST SELLER',
    style: 'ONS BOWIE SOFTSHELL BOMBER',
    smv: 26.96,
    daysRun: 3,
    wh: 10.00,
    mp: 62,
    dayTarget: 690,
    dayProduction: 470,
    planEff: 50,
    achvEff: 34,
    effPrf: 68,
    availableMin: 37200,
    produceMin: 12671
  },
  {
    lineNo: '10',
    chief: 'OMOL',
    buyer: 'H&M',
    style: 'QUINN LIGHT BOMBER JKT',
    smv: 28.49,
    daysRun: 4,
    wh: 10.00,
    mp: 62,
    dayTarget: 783,
    dayProduction: 329,
    planEff: 60,
    achvEff: 25,
    effPrf: 42,
    availableMin: 37200,
    produceMin: 9373
  },
  {
    lineNo: '11',
    chief: 'SOHEL',
    buyer: 'H&M',
    style: 'DANNY MID PADDED JKT',
    smv: 46.76,
    daysRun: 40,
    wh: 10.00,
    mp: 60,
    dayTarget: 539,
    dayProduction: 530,
    planEff: 70,
    achvEff: 69,
    effPrf: 98,
    availableMin: 36000,
    produceMin: 24783
  },
  {
    lineNo: '12',
    chief: 'ANAMUL',
    buyer: 'CLINTON',
    style: 'CB2703 8415',
    smv: 16.45,
    daysRun: 5,
    wh: 10.00,
    mp: 60,
    dayTarget: 1313,
    dayProduction: 554,
    planEff: 60,
    achvEff: 25,
    effPrf: 42,
    availableMin: 36000,
    produceMin: 9113
  },

  // KARNOPHULI FLOOR (Lines 13 - 17)
  {
    lineNo: '13',
    chief: 'NEWAS ALAM',
    buyer: 'C&A',
    style: '2264499 FUTURE SENSE JKT',
    smv: 30.97,
    daysRun: 9,
    wh: 11.00,
    mp: 57,
    dayTarget: 729,
    dayProduction: 400,
    planEff: 60,
    achvEff: 33,
    effPrf: 55,
    availableMin: 37620,
    produceMin: 12388
  },
  {
    lineNo: '14',
    chief: 'SHOHAG MIA',
    buyer: 'OTCF',
    style: 'JAF 1147',
    smv: 43.65,
    daysRun: 10,
    wh: 10.00,
    mp: 58,
    dayTarget: 478,
    dayProduction: 350,
    planEff: 60,
    achvEff: 44,
    effPrf: 73,
    availableMin: 34800,
    produceMin: 15278
  },
  {
    lineNo: '15',
    chief: 'SOHIDUL',
    buyer: 'BEST SELLER',
    style: 'LANDO LIFE BOMBER',
    smv: 35.75,
    daysRun: 18,
    wh: 10.00,
    mp: 60,
    dayTarget: 604,
    dayProduction: 570,
    planEff: 60,
    achvEff: 57,
    effPrf: 94,
    availableMin: 36000,
    produceMin: 20378
  },
  {
    lineNo: '16',
    chief: 'PABEL',
    buyer: 'C&A',
    style: '2268995 BASIC PADDED BOMBER',
    smv: 40.47,
    daysRun: 27,
    wh: 10.00,
    mp: 59,
    dayTarget: 525,
    dayProduction: 126,
    planEff: 60,
    achvEff: 14,
    effPrf: 24,
    availableMin: 35400,
    produceMin: 5099
  },
  {
    lineNo: '17',
    chief: 'EMON',
    buyer: 'OTCF',
    style: 'JAF 1156',
    smv: 45.20,
    daysRun: 11,
    wh: 10.00,
    mp: 58,
    dayTarget: 462,
    dayProduction: 300,
    planEff: 60,
    achvEff: 39,
    effPrf: 65,
    availableMin: 34800,
    produceMin: 13560
  },

  // KOROTOYA FLOOR (Lines 18 - 23)
  {
    lineNo: '18',
    chief: 'BABU',
    buyer: 'OTCF',
    style: 'JAM 433',
    smv: 42.03,
    daysRun: 26,
    wh: 10.00,
    mp: 63,
    dayTarget: 540,
    dayProduction: 300,
    planEff: 60,
    achvEff: 33,
    effPrf: 56,
    availableMin: 37800,
    produceMin: 12609
  },
  {
    lineNo: '19',
    chief: 'ROBIUL',
    buyer: 'C&A',
    style: '2264857 TG 30274306',
    smv: 31.45,
    daysRun: 8,
    wh: 10.00,
    mp: 63,
    dayTarget: 721,
    dayProduction: 322,
    planEff: 60,
    achvEff: 27,
    effPrf: 45,
    availableMin: 37800,
    produceMin: 10127
  },
  {
    lineNo: '20',
    chief: 'SAIDUR',
    buyer: 'C&A',
    style: '2268450 TB30274205 B',
    smv: 32.35,
    daysRun: 5,
    wh: 10.00,
    mp: 60,
    dayTarget: 668,
    dayProduction: 350,
    planEff: 60,
    achvEff: 31,
    effPrf: 52,
    availableMin: 36000,
    produceMin: 11323
  },
  {
    lineNo: '21',
    chief: 'ARIFUL ISLAM',
    buyer: 'C&A',
    style: '2268450 TB30274205 B',
    smv: 32.35,
    daysRun: 5,
    wh: 10.00,
    mp: 60,
    dayTarget: 668,
    dayProduction: 160,
    planEff: 60,
    achvEff: 14,
    effPrf: 24,
    availableMin: 36000,
    produceMin: 5176
  },
  {
    lineNo: '22',
    chief: 'SHAKIL',
    buyer: 'C&A',
    style: '2264499 FUTURE SENSE JKT',
    smv: 30.97,
    daysRun: 17,
    wh: 11.00,
    mp: 56,
    dayTarget: 716,
    dayProduction: 330,
    planEff: 60,
    achvEff: 28,
    effPrf: 46,
    availableMin: 36960,
    produceMin: 10220
  },
  {
    lineNo: '23',
    chief: 'SAMIM',
    buyer: 'C&A',
    style: '2264854 TG 15244303',
    smv: 28.45,
    daysRun: 7,
    wh: 10.00,
    mp: 55,
    dayTarget: 696,
    dayProduction: 390,
    planEff: 60,
    achvEff: 34,
    effPrf: 56,
    availableMin: 33000,
    produceMin: 11096,
    remarks: '2268995 BASIC PADDED BOMBER 3 pcs'
  },

  // SHITALOKSHYA FLOOR (Lines 24 - 29)
  {
    lineNo: '24',
    chief: 'Nur Alam',
    buyer: 'C&A',
    style: '2264855 TG 15274304',
    smv: 32.09,
    daysRun: 9,
    wh: 10.00,
    mp: 59,
    dayTarget: 662,
    dayProduction: 400,
    planEff: 60,
    achvEff: 36,
    effPrf: 60,
    availableMin: 35400,
    produceMin: 12836
  },
  {
    lineNo: '25',
    chief: 'JAFOR',
    buyer: 'C&A',
    style: '2264499 FUTURE SENSE JKT',
    smv: 30.97,
    daysRun: 9,
    wh: 11.00,
    mp: 57,
    dayTarget: 729,
    dayProduction: 323,
    planEff: 60,
    achvEff: 27,
    effPrf: 44,
    availableMin: 37620,
    produceMin: 10003
  },
  {
    lineNo: '26',
    chief: 'BADOL',
    buyer: 'C&A',
    style: '2264857 TG 30274306',
    smv: 31.45,
    daysRun: 7,
    wh: 10.00,
    mp: 58,
    dayTarget: 664,
    dayProduction: 270,
    planEff: 60,
    achvEff: 24,
    effPrf: 41,
    availableMin: 34800,
    produceMin: 8492
  },
  {
    lineNo: '27',
    chief: 'NAHIDUL',
    buyer: 'C&A',
    style: '2264499 FUTURE SENSE JKT',
    smv: 30.97,
    daysRun: 15,
    wh: 10.98,
    mp: 58,
    dayTarget: 740,
    dayProduction: 228,
    planEff: 60,
    achvEff: 18,
    effPrf: 31,
    availableMin: 38220,
    produceMin: 7061
  },
  {
    lineNo: '28',
    chief: 'Kabir',
    buyer: 'C&A',
    style: '2264857 TG 30274306',
    smv: 31.45,
    daysRun: 7,
    wh: 10.00,
    mp: 58,
    dayTarget: 664,
    dayProduction: 290,
    planEff: 60,
    achvEff: 26,
    effPrf: 44,
    availableMin: 34800,
    produceMin: 9121
  },
  {
    lineNo: '29',
    chief: 'SAMIM',
    buyer: 'C&A',
    style: '2264499 FUTURE SENSE JKT',
    smv: 30.97,
    daysRun: 16,
    wh: 11.00,
    mp: 57,
    dayTarget: 729,
    dayProduction: 199,
    planEff: 60,
    achvEff: 16,
    effPrf: 27,
    availableMin: 37620,
    produceMin: 6163
  },

  // TURAG FLOOR (Lines 30 - 34)
  {
    lineNo: '30',
    chief: 'ASHRAFUL',
    buyer: 'OTCF',
    style: 'JAM 1151',
    smv: 50.97,
    daysRun: 20,
    wh: 9.20,
    mp: 60,
    dayTarget: 390,
    dayProduction: 150,
    planEff: 60,
    achvEff: 23,
    effPrf: 38,
    availableMin: 33120,
    produceMin: 7646
  },
  {
    lineNo: '31',
    chief: 'FARUK',
    buyer: 'OTCF',
    style: 'JAM 1152',
    smv: 54.91,
    daysRun: 21,
    wh: 8.97,
    mp: 60,
    dayTarget: 353,
    dayProduction: 200,
    planEff: 60,
    achvEff: 34,
    effPrf: 57,
    availableMin: 32280,
    produceMin: 10982
  },
  {
    lineNo: '32',
    chief: 'BABU MIYA',
    buyer: 'OTCF',
    style: 'JAM 466',
    smv: 43.03,
    daysRun: 26,
    wh: 8.88,
    mp: 59,
    dayTarget: 438,
    dayProduction: 270,
    planEff: 60,
    achvEff: 37,
    effPrf: 62,
    availableMin: 31440,
    produceMin: 11618
  },
  {
    lineNo: '33',
    chief: 'ALAMIN',
    buyer: 'OTCF',
    style: 'JAF 472',
    smv: 37.35,
    daysRun: 11,
    wh: 9.43,
    mp: 60,
    dayTarget: 546,
    dayProduction: 185,
    planEff: 60,
    achvEff: 20,
    effPrf: 34,
    availableMin: 33960,
    produceMin: 6910
  },
  {
    lineNo: '34',
    chief: 'RAJIB',
    buyer: 'OTCF',
    style: 'JAM 466',
    smv: 36.46,
    daysRun: 8,
    wh: 9.33,
    mp: 45,
    dayTarget: 415,
    dayProduction: 96,
    planEff: 60,
    achvEff: 14,
    effPrf: 23,
    availableMin: 25200,
    produceMin: 3500
  }
];

export const DEBONAIR_SEPTEMBER_19_LINES: LineEntry[] = RAW_ROWS_SEP_19.map((row, idx) =>
  buildDebonairLineEntry(1900 + idx + 1, DEBONAIR_SEPTEMBER_19_DATE, row)
);
